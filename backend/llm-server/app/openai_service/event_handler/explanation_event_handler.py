...
class ExplanationEventHandler(AsyncAssistantEventHandler):
    def __init__(self, note_id, chunk_index):
        super().__init__()
    ...
    async def on_text_created(self, text: Text) -> None:
        logger.info(f"explanation generation started for video id: {self.note_id}")
    ...
    async def on_message_done(self, message):
        ...
        # 디렉토리 경로 생성
        explanation_dir = f"capstone_storage/{self.note_id}/explanation"
        os.makedirs(explanation_dir, exist_ok=True)  # 폴더가 없으면 생성

        # 생성된 설명문을 파일로 저장
        explanation_file_path = f"{explanation_dir}/explanation_{self.note_id}_{self.chunk_index}.txt"
        try:
