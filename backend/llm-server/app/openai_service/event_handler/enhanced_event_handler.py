...
class EnhancedExplanationEventHandler(AsyncAssistantEventHandler):
    ...
    async def on_message_done(self, message):
        # 설명문 생성 완료 시 호출
        logger.info(f"chunk_{self.chunk_index}에 대한 피드백 반영 설명문 생성 완료")
        # 디렉토리 경로 생성
        explanation_dir = f"capstone_storage/{self.note_id}/enhanced_explanation"
        os.makedirs(explanation_dir, exist_ok=True)  # 폴더가 없으면 생성
        # 생성된 설명문을 파일로 저장
        explanation_file_path = f"{explanation_dir}/enhanced_explanation_{self.note_id}_{self.chunk_index}.txt"
        try:
            with open(explanation_file_path, "w") as f:
                f.write(self.enhanced_explanation_data)
            logger.info(f"피드백 반영 설명문이 {explanation_file_path}에 저장되었습니다.")
        ...
        try:
            asyncio.create_task(
                self.kafka_producer.send_message(self.kafka_topic, message=complete_message.model_dump_json()))
