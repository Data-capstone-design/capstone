...
class EnhancedExplanationEventHandler(AsyncAssistantEventHandler):
    def __init__(self, thread_id, video_id ,chunk_index=None, kafka_producer=None,
    ...
    async def on_message_done(self, message):
        ...
        # 디렉토리 경로 생성
        explanation_dir = f"capstone_storage/{self.video_id}/enhanced_explanation"
        os.makedirs(explanation_dir, exist_ok=True)  # 폴더가 없으면 생성
        # 생성된 설명문을 파일로 저장
        explanation_file_path = f"{explanation_dir}/enhanced_explanation_{self.video_id}_{self.chunk_index}.txt"
        try:
