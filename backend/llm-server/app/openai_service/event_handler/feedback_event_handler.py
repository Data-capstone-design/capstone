...
class FeedbackEventHandler(AsyncAssistantEventHandler):
    def __init__(self, thread_id, chunk_index, note_id):
        super().__init__()
    ...
    async def on_text_created(self, text: Text) -> None:
        logger.info(f"feedback generation started for videoid: {self.note_id}- chunk_index: {self.chunk_index}")
    ...
    async def on_message_done(self, message):
        ...
        # 디렉터리가 존재하지 않을 경우 생성
        feedback_dir = f"capstone_storage/{self.note_id}/feedback"
        os.makedirs(feedback_dir, exist_ok=True)

        # 생성된 피드백을 파일로 저장
        feedback_file_path = f"{feedback_dir}/feedback_{self.note_id}_{self.chunk_index}.txt"
        try:
